...
Использование:
    python scripts/build_skill.py --output dist/swc-manajet-expert.skill
...
def build_skill_dir(repo_root: Path, version: str, build_date: str) -> Path:
    """Собирает папку swc-manajet-expert/ со всем содержимым."""
    build_dir = repo_root / "build" / "swc-manajet-expert"
    ...
    with zipfile.ZipFile(output_path, "w", zipfile.ZIP_DEFLATED) as zf:
        for file_path in sorted(build_dir.rglob("*")):
            if file_path.is_file():
                ...
                # swc-manajet-expert/ была корнем внутри архива
...
def main() -> int:
    ...
    parser.add_argument("--output", default="dist/swc-manajet-expert.skill",
    ...
    print(f"🔨 Сборка swc-manajet-expert v{version} (build date: {build_date})")
